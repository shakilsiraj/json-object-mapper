 ...
 */
export const DeserializeSimpleType = (
  instance: Object,
  instanceKey: string,
  type: any,
  json: any,
  ...
  try {
    ...
    throw new JsonConverstionError(
      `Property '${instanceKey}' of ${instance.constructor["name"]} does not match datatype of ${jsonKey}`,
      json
    );
  }
};
 ...
 */
export const DeserializeDateType = (
  instance: Object,
  instanceKey: string,
  type: any,
  json: any,
  jsonKey: string
): Array<ConversionFunctionStructure> => {
  try {
 ...
 */
export let DeserializeArrayType = (
  instance: Object,
  instanceKey: string,
  type: any,
  json: Object,
    ...
    for (let i = 0; i < jsonArraySize; i++) {
      if (jsonObject[i]) {
        ...
        if (!isSimpleType(typeName)) {
          ...
          arrayInstance.push(
            conversionFunctions[Constants.FROM_ARRAY](jsonObject[i], typeName)
          );
        }
      }
    }
...
export const DeserializeComplexType = (
  ...
  objectKeys = objectKeys.concat(
    (
      Reflect.getMetadata(METADATA_JSON_PROPERTIES_NAME, objectInstance) || []
    ).filter((item: string) => {
      if (
        objectInstance.constructor.prototype.hasOwnProperty(item) &&
        Object.getOwnPropertyDescriptor(
          objectInstance.constructor.prototype,
          item
        ).set === undefined
      ) {
  ...
  objectKeys.forEach((key: string) => {
     ...
     */
    let metadata: JsonPropertyDecoratorMetadata = getJsonPropertyDecoratorMetadata(
      objectInstance,
      key
    );
      ...
      if (metadata.required && json[metadata.name] === undefined) {
        throw new JsonConverstionError(
          `JSON structure does have have required property '${
            metadata.name
          }' as required by '${getTypeNameFromInstance(
            objectInstance
          )}[${key}]`,
          json
        );
          ...
          if (!isArrayType(objectInstance, key)) {
            ...
            const typeName =
              metadata.type != undefined
                ? getTypeNameFromInstance(metadata.type)
                : getTypeName(objectInstance, key);
            if (!isSimpleType(typeName)) {
            ...
            const moreFunctions: Array<ConversionFunctionStructure> = conversionFunctions[
              Constants.ARRAY_TYPE
            ](objectInstance, key, metadata.type, json, jsonKeyName);
