...
describe("Testing deserialize functions", () => {
  it("Testing Class type with no annotations and 0 children", () => {
    ...
    const nameTypeWithoutAnnotationsJson = {
      firstName: "John",
      middleName: "P",
      lastName: "Doe"
    };

    const processedDto: NameTypeWithoutAnnotations = ObjectMapper.deserialize(
      NameTypeWithoutAnnotations,
...
describe("Testing serialize array function", () => {
  it("Testing array serialization", () => {
    ...
    expect(serializedString).toBe(
      `[{"id":1,"location":"Canberra"},{"id":2,"location":"Sydney"},{"id":3,"location":"Melbourne"}]`
    );
  });
});
...
describe("Testing deserialize array function", () => {
  ...
  it("Deserialize array with one null value", () => {
    ...
    const json = JSON.parse(
      '[{"id":1,"location":"Canberra"},{"id":2,"location":"Sydney"}, {"id":3,"location":"Melbourne"}, null]'
    );
    const eventsArray: Event[] = ObjectMapper.deserializeArray(Event, json);
    expect(eventsArray.length > 0);
    expect(eventsArray[0].id).toBe(1);
...
describe("Testing serialize functions", () => {
  it("Testing Class type with no annotations and 0 children", () => {
    ...
    expect(stringrified).toBe(
      `{"firstName":"John","middleName":"P","lastName":"Doe"}`
    );
  });
  ...
  it("Testing Class type with no annotations and an array", () => {
    ...
    expect(stringrified).toBe(
      `{"firstName":"John","middleName":"P","lastName":"Doe","dateOfBirth":1483142400000,"AKA":["John","Doe","JohnDoe","JohnPDoe"]}`
    );
  });
  ...
  it("Test all simple type properties", () => {
    ...
    const testInstance: SimpleRoster = ObjectMapper.deserialize(
      SimpleRoster,
      json
    );
...
describe("Misc tests", () => {
  it("Tesing Dto with functions", () => {
    ...
    expect(testInstance.isAvailableToday()).toBe(
      new Date().getDay() % 6 === 0 ? false : true
    );
  });
  ...
  it("Testing enum ", () => {
    ...
    class Workday {
      @JsonProperty({
        type: Days,
        deserializer: DaysEnumSerializerDeserializer,
        serializer: DaysEnumSerializerDeserializer
      })
  ...
  it("Testing deserializer and serializer instances", () => {
    ...
    const instance3: TestDeserializer = getOrCreateDeserializer(
      TestDeserializer
    );
    const instance4: TestDeserializer = getOrCreateDeserializer(
      TestDeserializer
    );
...
describe("Testing JsonIgnore decorator", () => {
  it("Testing JsonIgnore serialization", () => {
    ...
    const serializedString: String = ObjectMapper.serialize(
      new Event(1, "Canberra", "new")
    );
...
describe("Testing JsonIgnore decorator", () => {
  ...
  it("{ ignoreDeserialization: true } should not use specified deserializer", () => {
