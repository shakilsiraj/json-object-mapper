...
import { Deserializer, JsonProperty } from "../main/DecoratorMetadata";
...
describe("Testing Map De-serializer", () => {
  it("Test Map Serialization", () => {
      ...
      deserialize = (value: any): any => {
        ...
        if (value) {
          Object.keys(value).forEach((key: String) => {
            mapToReturn.set(key, value[String(key)]);
          });
        }
        return mapToReturn;
      };
    ...
    const servletConfig: ServletConfig = ObjectMapper.deserialize(
      ServletConfig,
      dataset
    );
